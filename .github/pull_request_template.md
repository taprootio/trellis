<!--
Title format: TRLxxxx — imperative summary (<= 72 chars)
  e.g. "TRL0002 — implement the config-driven generator core"
For a branch spanning several items, lead with the primary id and name the rest
in the body. Generate a conforming title + description with the shortcut at
docs/playbooks/pr-draft.md.
No AI/co-author attribution: no Co-Authored-By trailers or "Generated with" footers.
-->

## Summary

<!-- 1–3 sentences: what this PR does and why. -->

## Task

<!-- Link the backlog item(s): TRLxxxx. Note any that are closed out here. -->

## Changes

<!-- The notable changes, grouped if helpful. Describe what shipped, not a file list. -->

## Verification

- [ ] `npm run backlog:check` passes
- [ ] Touched docs / spec updated
<!-- Add the commands you ran and their result. -->

## Follow-ups

<!-- New TRLs spun off, or "none". -->
